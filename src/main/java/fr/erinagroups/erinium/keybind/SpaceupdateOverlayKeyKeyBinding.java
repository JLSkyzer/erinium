...
package fr.erinagroups.erinium.keybind;
...
public class SpaceupdateOverlayKeyKeyBinding extends EriniumModElements.ModElement {
		...
		if (Minecraft.getInstance().currentScreen == null) {
			if (event.getKey() == keys.getKey().getKeyCode()) {
				if (event.getAction() == GLFW.GLFW_PRESS) {
					EriniumMod.PACKET_HANDLER.sendToServer(new KeyBindingPressedMessage(0, 0));
	...
	private static void pressAction(PlayerEntity entity, int type, int pressedms) {
		...
		if (type == 0) {
			...
			SpaceupdateOverlayKeyOnKeyPressedProcedure.executeProcedure(Stream.of(new AbstractMap.SimpleEntry<>("entity", entity))
