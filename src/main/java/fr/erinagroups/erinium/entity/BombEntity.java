...
import fr.erinagroups.erinium.procedures.BombProjectileHitsBlockProcedure;
...
public class BombEntity extends AbstractArrow implements ItemSupplier {
	public BombEntity(PlayMessages.SpawnEntity packet, Level world) {
		super(EriniumModEntities.BOMB.get(), world);
	}
	...
	public ItemStack getItem() {
		return new ItemStack(EriniumModBlocks.HOUSE_1_BLOCK.get());
	}
	...
	public static BombEntity shoot(Level world, LivingEntity entity, RandomSource random, float power, double damage, int knockback) {
		BombEntity entityarrow = new BombEntity(EriniumModEntities.BOMB.get(), entity, world);
	...
	public static BombEntity shoot(LivingEntity entity, LivingEntity target) {
		BombEntity entityarrow = new BombEntity(EriniumModEntities.BOMB.get(), entity, entity.level());
