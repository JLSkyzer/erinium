...
public class WizardTowerFeature extends Feature<NoneFeatureConfiguration> {
	...
	public WizardTowerFeature() {
		super(NoneFeatureConfiguration.CODEC);
		base_blocks = List.of(EriniumModBlocks.SCINING_SAND.get(), Blocks.GRASS_BLOCK, Blocks.DIRT);
	}
	...
	public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
		if (!generate_dimensions.contains(context.level().getLevel().dimension()))
			return false;
		if (template == null)
			template = context.level().getLevel().getStructureManager().getOrCreate(new ResourceLocation("erinium", "wizard_tower"));
		if (template == null)
			return false;
